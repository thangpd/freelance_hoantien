<?php /* Template Name: xemhoahonggioithieu */ ?>
<?php get_header(); ?>
<?php
...
<br />
		...
		<div style="text-align: center;"><span style="font-size:16px;"><span style="font-size:22px;"><span style="color:#008000;"><span style="font-family:times new roman,times,serif;"><strong>DANH SÁCH ĐƠN HÀNG CỦA NGƯỜI ĐƯỢC BẠN GIỚI THIỆU</strong></span></span></span></span></div>
...
<div style="text-align: center;"><span style="font-size:16px;"><span style="color:#000000;"><span style="font-family:times new roman,times,serif;">(Bạn chỉ có thể xem dữ liệu từng tháng, ví dụ chọn ngày <strong>01/04 - 30/04</strong> hoặc <strong>01/07 - 31/07</strong>)</span></span></span></div>	
	...
	<input type="hidden" name="idnguoigioithieu" value="<?php echo $_GET["idnguoigioithieu"]; ?>">
TỪ NGÀY:
...
<form action="https://hoantienmuasam.com/hoa-hong-nguoi-duoc-gioi-thieu/" method="GET" style="text-align:center;">
<select class="idngtsl" name="idnguoigioithieu" style="    width: 250px;">
<option value="nouser">
			CHỌN MÃ NGƯỜI DÙNG MUỐN XEM
			</option>
	<?php
	$args = array(
    'role'    => 'wpas_user',
...
foreach($users as $user_id){
        $idchinh=get_user_meta($user_id->ID,"ma_gioi_thieu",true);
		echo $idchinh;
		if($idchinh==$id_user)
		{
			$idngt=$user_id->ID;
			if($idngt==$_GET["idnguoigioithieu"])
			{
				?>
				<option value="<?php echo $user_id->ID; ?>" selected>
			<?php echo $user_id->ID; ?>
			</option>
				<?php
			}
...
<?php
$idgioithieu=$_GET["idnguoigioithieu"];
?>
